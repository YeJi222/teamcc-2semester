...
exports.image_upload = function(request, response, rand_url){
  ...
  if(request.file !== undefined ){
    ...
    db.query(`SELECT * FROM url WHERE url=?`,[rand_url],function(error1,admin_id){ if(error1){ throw error1; }
      db.query(`INSERT INTO cc VALUE('${admin_id[0].admin_id}',${request.body.team_num},${request.body.actid},${request.body.actindex},'${base64image}',${request.file.size},'${request.file.mimetype}')`, function(error2,cc){
        if(error2){
          var alert = `
            <script>
              alert('등록 오류!! ${request.file.buffer} ${admin_id[0].admin_id} ${request.body.team_num} ${request.body.actid} ${request.body.actindex} ${request.file.size} ${request.file.mimetype}');
              location.href = '/teamcc/user?id=${rand_url}';
            </script>
          `;
